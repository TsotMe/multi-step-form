...
<template>
<div class="h-full mt-[35px] flex flex-col gap-6">
  <div class="bg-[#F8F9FF] rounded-lg pt-4 pb-6 px-6">

    <div :class="`${!hideDivider ? 'mb-6' : ''} flex items-center justify-between`">
      <div class="flex flex-col gap-[7px]">
        <h3 class="text-base text-[#022959]">{{ planName }}</h3>
        <button class="w-fit h-fit text-[#9699AA] transition-all duration-200 underline hover:text-[#483EFF]"
                @click="emit('goToSecondStep')">
          Change
        </button>
      </div>
      <span class="text-base text-[#022959] font-ubuntu-bold">{{ planPrice }}</span>
    </div>

    <div v-if="!hideDivider" class="h-[1px] bg-[#9699AA] opacity-[0.2043] mb-4"/>

    <div v-if="onlineService.isSelected"
         :class="`${largerStorage.isSelected ? 'mb-4' : ''} flex items-center justify-between`">
      <h4 class="text-[#9699AA]">Online service</h4>
      <span class="text-[#022959]">{{ onlineService.price }}</span>
    </div>

    <div v-if="largerStorage.isSelected"
         :class="`${customizableProfile.isSelected ? 'mb-4' : ''
    } flex items-center justify-between`">
      <h4 class="text-[#9699AA]">Larger storage</h4>
      <span class="text-[#022959]">{{ largerStorage.price }}</span>
    </div>

    <div v-if="customizableProfile.isSelected"
         class="flex items-center justify-between">
      <h4 class="text-[#9699AA]">Customizable profile</h4>
      <span class="text-[#022959]">{{ customizableProfile.price }}</span>
    </div>
  </div>

  <div class="flex items-center justify-between px-6">
    <h3 class="text-[#9699AA]">{{ totalTitle }}</h3>
    <span class="text-[#483EFF] text-xl font-ubuntu-bold">{{ totalPrice }}</span>
  </div>
</div>
</template>
